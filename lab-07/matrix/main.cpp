...
// reading from files should be in a separate function and call twice for 2 files
    while (std::getline(inputFile1, temp))
    {
        std::istringstream iss(temp);
        std::vector<double> row;//large vector
        ...
        // m1.push_back(row);//expansive copy of large vector
        m1.emplace_back(std::move(row));//only changing of the ownership of the large vector
    }
